...
public class MonthView extends View implements ValueAnimator.AnimatorUpdateListener {
	...
	protected OnDateSelected m_OnDateSelected;
	private Date m_VailidBeginDate;
	private Date m_VailidEndingDate;
	private int m_VailidDateColor;
        ...
        for (int i = 0; i < mRegion.length; i++) {
            for (int j = 0; j < mRegion[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    final String strDate = currentYear + "-" + currentMonth + "-" + mCalendarBiz.getGregorianCreated().get(index)[i][j];
                    Date date = null;
					try {
						date = m_Sdf.parse(strDate);
					} catch (ParseException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
                    if (dateSelected.contains(strDate)) {
                        dateSelected.remove(strDate);

                        BGCircle circle = circlesAppear.get(strDate);
                        ...
                        animScale.addListener(new AnimatorListenerAdapter() {
                            ...
                            public void onAnimationEnd(Animator animation) {
                                circlesDisappear.remove(strDate);
                            }

                        });
                        animScale.start();

                        circlesDisappear.put(strDate, circle);

                        circlesAppear.remove(strDate);
                    } else if(isVailidDate(date)){
                        dateSelected.add(strDate);
                        ...
                        animScale4.addListener(new AnimatorListenerAdapter() {
                        ...
                        circlesAppear.put(strDate, circle);
                    }
                }
            }
        }
	...
	private boolean isVailidDate(Date date) {
		if(m_VailidBeginDate == null || m_VailidEndingDate == null){
			return true;
		}
		return (date.after(m_VailidBeginDate) || date.compareTo(m_VailidBeginDate) == 0) && date.before(m_VailidEndingDate);
	}
    ...
    private void drawMonth(Canvas canvas, float offsetX, int year, int month) {
        ...
        for (int i = 0; i < gregorianCurrent.length; i++) {
            for (int j = 0; j < gregorianCurrent[i].length; j++) {
                ...
                try {
					Date date = m_Sdf.parse(year+"-"+month+"-"+str);
					if(!isVailidDate(date)){
						mTextPaint.setColor(Color.GRAY);
					} else{
						if(m_VailidDateColor != 0){							
							mTextPaint.setColor(m_VailidDateColor);
						} else{
							mTextPaint.setColor(Color.BLACK);
						}
					}
	...
	public void setOnDateSelectedListener(OnDateSelected pOnDateSelected) {
		// TODO Auto-generated method stub
		m_OnDateSelected = pOnDateSelected;
	}

	public void setVailidDays(Date pVailidBeginDate, Date pVailidEndingDate, int pColor) {
		// TODO Auto-generated method stub
		m_VailidBeginDate = pVailidBeginDate;
		m_VailidEndingDate = pVailidEndingDate;
		m_VailidDateColor = pColor;
	}
}
